...
export interface CollateralPosition {
  ...
  collateralValueUsd?: number; // Valeur USD directe depuis DeBank
  debtToken: string;
  debtAmount: number;
  debtValueUsd?: number; // Valeur USD directe depuis DeBank
  borrowApr: number;
...
function mapPortfolioItemToPosition(
  ...
  return {
    ...
    collateralAmount,
    collateralPriceUsd,
    collateralValueUsd: collatUsd, // Valeur USD directe depuis DeBank
    debtToken: debtTokenSymbol,
    debtAmount,
    debtValueUsd: debtUsd, // Valeur USD directe depuis DeBank
...
export async function buildCollateralClientFromDeBank(
  ...
  for (const protocol of protoList || []) {
    ...
    if (Array.isArray(allowedProtocols) && allowedProtocols.length > 0) {
      ...
      const protocolMatches = allowedProtocols.some(p => {
